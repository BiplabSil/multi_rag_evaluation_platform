...
def test_get_qdrant_client():
    ...
    with patch("core.vector_store.QdrantClient") as mock_client:
        with patch("core.vector_store._settings") as mock_settings:
...
def test_ensure_collection_skips_existing(mock_qdrant_client):
...
def test_search_by_metadata(mock_qdrant_client):
...
def test_delete_by_metadata(mock_qdrant_client):
...
def test_delete_by_metadata_no_results(mock_qdrant_client):
